	...
	if(!strcmp($ucap,$_SESSION['captcha'])) {
				...
				if ($retrive_type === "true"){
					if($retrive_email === "true") {
								if(!$login->doReset($umail,$lang,$type)) {
									$error = "<br> SYSTEM RESET ERROR!";
								} else {
									$_SESSION['reset_sent'] = $reset_mail;
								}
						} else {
...
<form action="" method="post" name="Reset_Form">
  ...
  <div class="container" style="text-align:center;">
		...
		<br />
    <button name="btn-login" type="submit" value="Reset"><?php echo ucfirst($reset) ?></button>
  </div>
</form>
</main>
